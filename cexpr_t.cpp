...
make_carray(F f) {
 return do_make_carray<I0>(f, typename make_indices<N>::type());
}
...
float getNtmp(int i) {
  ...
  constexpr auto v = make_carray<nbin>(ntmp);
  return v[i];
}
...
int main() {
  ...
  */
  //  constexpr auto v = make<nbin>(ntmp); // OK


  for(int i=0; i<nbin; i++)
    //   std::cout << i << ": " <<  ntmp_ori[i] << " " << v[i] << std::endl;
    std::cout << i << ": " <<  ntmp_ori[i] << " " << getNtmp(i) << std::endl;
