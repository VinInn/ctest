...
class Fmat43
{
 ...
 public:
  ...
  inline Fvec4 toGlobalP(Fvec4 & lp)
  {
   __m128 m0 = _mm_load_ps(&values[0]);
   __m128 m1 = _mm_load_ps(&values[4]);
   __m128 m2 = _mm_load_ps(&values[8]);
   ...
   float r[4];
   _mm_store_ps(r, rs);
   return Fvec4(r[0]+_p0,r[1]+_p1,r[2]+_p2,0);
  }

  inline Fvec4 toGlobalV(Fvec4 & lv) 
  {
   __m128 m0 = _mm_load_ps(&values[0]);
   __m128 m1 = _mm_load_ps(&values[4]);
   __m128 m2 = _mm_load_ps(&values[8]);
   ...
   float r[4];
   _mm_store_ps(r, rs);
   return Fvec4(r[0],r[1],r[2],0);
  }

  inline Fvec4 toLocalP(Fvec4 & gp) 
  {
  ...
  inline Fvec4 toLocalV(Fvec4 & gv) 
  {
   __m128 m0 = _mm_set_ps(0, _m20, _m10, _m00);
   __m128 m1 = _mm_set_ps(0, _m21, _m11, _m01);
   __m128 m2 = _mm_set_ps(0, _m22, _m12, _m02);
...
Fvec4 do_rotate(float theta_degrees, Fvec4 axis_to_norm, Fvec4 p, Fvec4 vect)
{
 ...
 float theta = theta_degrees*PI/180;
