...
inline
float as_logf(float x) {
  const static double ix[] = {
  ...
  if (__builtin_expect(ux < 1u<<23 || ux >= 0xffu<<23, 0)) {
    if (ux==0||ux==(1<<31)) return -__builtin_inff(); // +0.0 || -0.0
    unsigned inf_or_nan = ((ux>>23)&0xff) == 0xff, nan = inf_or_nan && (ux<<9);
    if (ux>>31 && !nan) return NAN; // std::nanf("-"); // __builtin_nanf("-");
  ...
  const static double c[] = {0x1p+0, -0x1p-1, 0x1.55555555030bcp-2, -0x1.ffffffff2b4e5p-3, 0x1.999b5076a42f2p-3, -0x1.55570c45a647dp-3};
  double z2 = z*z;
  double c0 = c[0] + z*c[1];
  double c2 = c[2] + z*c[3];
  double c4 = c[4] + z*c[5];
  ...
  if(__builtin_expect((res.u&0xfffffffl) == 0, 0)){
    if(xd.f + 0x1p-54 == xd.f - 0x1p-54){ // round-to-nearest mode
      ...
      else if(x == 0x1.bacb4ap+25f) return  0x1.1e0696p+4f;
      else if(x == 0x1.b121a6p+76f) return  0x1.a9a3f2p+5f;
      else if(x == 0x1.6351d8p+95f) return  0x1.08b512p+6f;
    }
  }
  return res.f;
}
