...
#include<math.h>
    ...
    double kernel_measureMonteCarlo(double min_time, Random R)
    {
        ...
        /* approx Mflops */
        result *= 1.0e-12/cycles; // make use of MC result
            ...
            double min_time, Random R)
    {
        ...
        int *col = (int*) aligned_alloc(64,sizeof(int)*nz);
        int *row = (int*) aligned_alloc(64,sizeof(int)*(N+1));
        int r=0;
        int cycles=1;
        ...
        while(1)
        {
            ...
            if (Stopwatch_read(Q) >= min_time  || fabs(y[0])>1.e38) break;
    ...
    double kernel_measureLU(int N, double min_time, Random R)
    {
        ...
        if ((pivot = (int *) aligned_alloc(64,N * sizeof(int))) == NULL) exit(1);
