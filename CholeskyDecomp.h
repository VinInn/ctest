...
public:
   ...
   /** @returns true if decomposition was successful */
   CMS_HOST_DEVICE bool ok() const { return fOk; }
   /// returns true if decomposition was successful
   /** @returns true if decomposition was successful */
   CMS_HOST_DEVICE operator bool() const { return fOk; }
    ...
    */
   template<class V>
   CMS_HOST_DEVICE bool Solve(V& rhs) const
   {
    ...
    */
   template<class M>
   CMS_HOST_DEVICE bool Invert(M& m) const
   {
    ...
    */
   template<typename G>
   CMS_HOST_DEVICE bool Invert(G* m) const
   {
      using CholeskyDecompHelpers::_inverter;
      using CholeskyDecompHelpers::PackedArrayAdapter;
      if (fOk) {
    ...
    */
   template<class M>
   CMS_HOST_DEVICE bool getL(M& m) const
   {
      if (!fOk) return false;
    ...
    */
   template<typename G>
   CMS_HOST_DEVICE bool getL(G* m) const
   {
      if (!fOk) return false;
      // copy L
    ...
    */
   template<class M>
   CMS_HOST_DEVICE bool getLi(M& m) const
   {
      if (!fOk) return false;
      for (unsigned i = 0; i < N; ++i) {
    ...
    */
   template<typename G>
   CMS_HOST_DEVICE bool getLi(G* m) const
   {
      if (!fOk) return false;
      // copy L
      for (unsigned i = 0; i < (N * (N + 1)) / 2; ++i)
...
namespace CholeskyDecompHelpers {
   ...
   public:
      /// constructor
      CMS_HOST_DEVICE PackedArrayAdapter(G* arr) : fArr(arr) {}
      /// read access to elements (make sure that j <= i)
      CMS_HOST_DEVICE const G operator()(unsigned i, unsigned j) const
      { return fArr[((i * (i + 1)) / 2) + j]; }
      /// write access to elements (make sure that j <= i)
      CMS_HOST_DEVICE G& operator()(unsigned i, unsigned j)
      { return fArr[((i * (i + 1)) / 2) + j]; }
   };
   ...
   template<class F, class M> struct _decomposerGenDim
   {
      ...
      /** @returns if the decomposition was successful */
      CMS_HOST_DEVICE bool operator()(F* dst, const M& src, unsigned N) const
      {
   ...
   template<class F, unsigned N, class M> struct _decomposer
   {
      /// method to do the decomposition
      /** @returns if the decomposition was successful */
      CMS_HOST_DEVICE bool operator()(F* dst, const M& src) const
   ...
   template<class F, class V> struct _solverGenDim
   {
      ...
      CMS_HOST_DEVICE void operator()(V& rhs, const F* l, unsigned N) const
      {
         // solve Ly = rhs
   ...
   {
      /// method to solve the linear system
      CMS_HOST_DEVICE void operator()(V& rhs, const F* l) const
   ...
   {
      /// method to do the inversion
      CMS_HOST_DEVICE inline void operator()(M& dst, const F* src) const
      {
