...
/*  a function is not a type, all functions are of the same type, no way gcc can optimized generic code like this
volatile float yyy;

template<typename F, typename T>
void  __attribute__((noinline)) sloop(T * __restrict__ s, T const * __restrict__ xx) {
    const int ls=16;
    for (int j=0; j < ls; ++j) { 
      s[j] = F(xx[j]);
    }
...
struct  scalarBench {
  ...
  int operator()(double xi) {
  //  typedef decltype(F(0)) T;
  int niter = 100000,i; 
  T x = (xi>10000) ? 0.75f : 0.5f; yyy=0;	
  const int ls=16;
  for (i=0; i < niter/4; ++i) {      
    T __attribute__ ((aligned(16))) s[ls]; T __attribute__ ((aligned(16))) xx[ls];
    for (int j=0; j < ls; ++j) xx[j] =x+T(j);
    // this requires --param max-completely-peel-times=1 to vectorize
    sloop<F,T>(s,xx);	
    x += 1e-6f; 
...
#define DECL_SCALAR_FN_BENCH(fn)                     \
  int bench_##fn(double xi) { return scalarBench<fn>(xi); }

*/

  // it requires --param max-completely-peel-times=1 to vectorize
 volatile float yyy;
#define DECL_SCALAR_FN_BENCH(fn)                     \
  void  sloop_##fn(float * __restrict__ s, float const * __restrict__ xx) { \
  const int ls=16; \
  for (int j=0; j < ls; ++j) { \
    s[j] = fn(xx[j]);  \
  } \
} \
  void  sloop_##fn(double * __restrict__ s, double const * __restrict__ xx) { \
  const int ls=16; \
  for (int j=0; j < ls; ++j) { \
    s[j] = fn(xx[j]);  \
  } \
} \
  void  sloop_##fn(long double * __restrict__ s, long double const * __restrict__ xx) { \
  const int ls=16; \
  for (int j=0; j < ls; ++j) { \
    s[j] = fn(xx[j]);  \
  } \
    ...
    \
  int bench_##fn(double xi) {                                 \
    typedef decltype(fn(0)) ltype; \
    int niter = 100000,i;                           \
    ltype x = (xi>10000) ? 0.75f : 0.5f; yyy=0;	       \
    const int ls=16;				       \
    for (i=0; i < niter/4; ++i) {                        \
    ltype __attribute__ ((aligned(16))) s[ls]; ltype __attribute__ ((aligned(16))) xx[ls]; \
      for (int j=0; j < ls; ++j) xx[j] =x+ltype(j); 	     \
      sloop_##fn(s,xx);				     \
     x += 1e-6f;                                  \
    for (int j=0; j < ls; ++j) yyy+=s[j];	    \
    }                                                \
...
#define DECL_VECTOR_FN_BENCH(fn)                                        \
  int bench_##fn(double xi) {                                                    \
    ...
    v4sf x = _mm_set_ps1( (xi>10000) ? 0.75f : 0.5f);			\
    for (i=0; i < niter; ++i) {                                         \
...
float  atanf1(float x) { return atan2f(x,1.f);}
double atand1(double x) { return atan2(x,1.);}


DECL_SCALAR_FN_BENCH(sinf);
DECL_SCALAR_FN_BENCH(cosf);
DECL_SCALAR_FN_BENCH(sinl);
DECL_SCALAR_FN_BENCH(cosl);
#ifdef HAVE_SINCOS_X86_FPU
DECL_SCALAR_FN_BENCH(stupid_sincos_x86_fpu);
#endif
DECL_SCALAR_FN_BENCH(ln16);

DECL_SCALAR_FN_BENCH(logf);
DECL_SCALAR_FN_BENCH(expf);
DECL_SCALAR_FN_BENCH(logl);
DECL_SCALAR_FN_BENCH(expl);

DECL_SCALAR_FN_BENCH(atanf1);
DECL_SCALAR_FN_BENCH(atand1);


inline float vsincosf(float x) { float s,c; cephes::sincosf(x,s,c); return s+c;}
inline float vatanf(float x) { return cephes::atan2f(x,1.f);}
inline float vexpf(float x) { return cephes::expf(x);}
...
DECL_SCALAR_FN_BENCH(vatanf);
DECL_SCALAR_FN_BENCH(vexpf);
...
void run_bench(const char *s, int (*fn)(double)) {
  printf("benching %20s ..", s); fflush(stdout);
  double t0 = uclock_sec(), t1=t0, tmax = 1.0;
  niterV = 0;
  do {
    niterV += fn(t0);
    t1 = uclock_sec();
...
#ifndef  REF_FREQ_MHZ
#define REF_FREQ_MHZ 3000.0
#endif
  printf(" -> %6.1f millions of vector evaluations/second -> %3.0f cycles/value on a %gMHz computer\n", floor(niterV/(t1-t0)/1e5)/10, (t1-t0)*REF_FREQ_MHZ*1e6/niterV/4, REF_FREQ_MHZ);
}
...
// #endif
  ...
  run_bench("ln16", bench_ln16);

  run_bench("atan2f", bench_atanf1);
  run_bench("atan2", bench_atand1);


  run_bench("cephes::sincosf", bench_vsincosf);
  run_bench("cephes::atan2f", bench_vatanf);
  run_bench("cephes::expf", bench_vexpf);
  ...
  run_bench("sinl", bench_sinl);
  run_bench("cosl", bench_cosl);
  run_bench("expl", bench_expl);
  run_bench("logl", bench_logl);
