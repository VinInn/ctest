...
_GLIBCXX_BEGIN_NAMESPACE_VERSION
  ...
  template<typename _UIntType, size_t __w,
	...
	typename std::enable_if<std::is_class<_Sseq>::value>::type
      ...
      inline void loop(int __N, __F & __fn) {
	int _trail = state_size - _M_p;
	if (__N<=_trail) {
	  for (int _i=0; _i!=__N; ++_i) __fn(_M_temper_one(_M_x[_M_p+_i]));
	  _M_p+=__N;
	  return;
	}
	
	for (int _i=_M_p; _i!=state_size; ++_i)  __fn(_M_temper_one(_M_x[_i]));
	int _outLoop = (__N-_trail)/state_size;
	for (int _j=0; _j!=_outLoop; ++_j)  {  
	  _M_gen_state();
	  for (int _i=0; _i!=state_size; ++_i)  __fn(_M_temper_one(_M_x[_i]));
	}
	_M_gen_state();
	_M_p = __N -_trail -  _outLoop*state_size;
       	for (int _i=0; _i!=_M_p; ++_i)  __fn(_M_temper_one(_M_x[_i]));
      }
      ...
      void operator()(result_type __restrict__ * __v, size_t __vsz);
      using array_type=std::array<result_type,state_size>;
      void operator()(array_type __restrict__ & __v);
      array_type get();
    ...
    private:
      inline void _M_gen_rand();
      inline void _M_gen_state();
      inline void _M_temper(_UIntType __restrict__ * __v);
      inline static  _UIntType _M_temper_one(_UIntType __x);

      _UIntType   _M_x[state_size];
      _UIntType   _M_r[state_size];
      size_t      _M_p;
    };
   ...
   * @f]
   *
   * The gsize of the state is @f$r@f$
