...
HOST_DEVICE_CONSTANT float f32_sinpi_7_k[] = { 0x1.921fb6p1f,  -0x1.4abbecp2f, 0x1.466b2p1f,  -0x1.2f5992p-1f };
...
HOST_DEVICE_CONSTANT float f32_cospi_6_k[] = { 0x1p0, -0x1.3bd3ap2, 0x1.03b132p2, -0x1.4ea1aep0};
...
std::tuple<float,float> f32_sincospi(float a)
{
  ...
  return { q==0 ? s :c, q==0 ? c :s };

}


constexpr float f32_sinpi(float x) {
 auto [s,c] = f32_sincospi(x);
 return s;
}

constexpr float f32_cospi(float x) {
 auto [s,c] = f32_sincospi(x);
 return c;
}
