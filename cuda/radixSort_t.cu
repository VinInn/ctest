...
struct RS<float> {
  ...
  static auto ud() { return type(-std::numeric_limits<T>::max()/2.f,std::numeric_limits<T>::max()/2.f);}
//  static auto ud() { return type(0,std::numeric_limits<T>::max()/2);}
  static constexpr int32_t imax = std::numeric_limits<int32_t>::max(); 
};
...
struct RS<float16_t> {
  using T = float16_t;
  using type = std::uniform_real_distribution<float>;
  static auto ud() { return type(-float(65504.f)/2.f,float(65504.f)/2.f);}
//  static auto ud() { return type(0,std::numeric_limits<T>::max()/2);}
  static constexpr int16_t imax = std::numeric_limits<int16_t>::max();
};
  ...
  for (int i=0; i<50; ++i) {
    ...
    for (long long j = 0; j < N; j++) v[j]=rgen(eng);
      ...
      for (uint64_t j = 0; j < N; j++) {
        unsigned long long tmp = j%imax;
        v[j]=T(tmp); if(j%2 && i%2) v[j]=-v[j];
      }
    }
    ...
    shorten(k1);shorten(k2);
     if (k1<k2)
      std::cout << ib << " not ordered at " << ind[j] << " : "
  		<< LL(a[ind[j]]) <<' '<< LL(a[ind[j-1]]) << std::endl;
  }
  if (!inds.empty()) {
    assert(0 == *inds.begin());
...
try{
  ...
  // go<float16_t,2,float16_t,double>(useShared);
