...
namespace cms {
  namespace cuda {
    ...
    public:
      ...
      using CountersOnly = OneToManyAssoc<ONES, I>;
	...
	if constexpr (Assoc::ctNOnes() < 0) {
      ...
      __host__ __device__ __forceinline__ void count(Counter b) {
        assert(b < nOnes());
        atomicIncrement(off[b]);
      }

      __host__ __device__ __forceinline__ void fill(Counter b, index_type j) {
        assert(b < nOnes());
      ...
      __host__ __device__ __forceinline__ void bulkFinalizeFill(AtomicPairCounter const &apc) {
        ...
        for (auto i = first; i < totOnes(); i += gridDim.x * blockDim.x) {
          off[i] = n;
        }
      }
      ...
      __host__ __device__ __forceinline__ void finalize(Counter *ws = nullptr) {
        ...
        blockPrefixScan(off, totOnes(), ws);
      ...
      FlexiStorage<Counters,ONES> off;
