...
void 
__forceinline__
warpPrefixScan(T const * __restrict__ ci, T * __restrict__ co, uint32_t i, uint32_t mask) {
...
void
__forceinline__
blockPrefixScan(T const * __restrict__ ci, T * __restrict__ co, uint32_t size, T* ws
...
#else
  co[0]=ci[0];
  for(uint32_t i=1; i<size; ++i) co[i]=ci[i]+co[i-1];
#endif
}
