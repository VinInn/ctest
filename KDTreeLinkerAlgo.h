...
class KDTreeLinkerAlgo
{
 ...
 private:
  ...
  void recSearch(int			current,
                 VALUE dimCurrMin, VALUE dimCurrMax,
                 VALUE dimOtherMin, VALUE dimOtherMax);
                                  ...
                                  VALUE dimOtherMin, VALUE dimOtherMax)
{
  ...
  while(true) {
    ...
    if(nodePool_.isLeaf(right)) {
      VALUE dimCurr = nodePool_.median[current];
      ...
      if((dimCurr >= dimCurrMin) & (dimCurr <= dimCurrMax)) {
        VALUE dimOther = nodePool_.dimOther[current];
      ...
      VALUE median = nodePool_.median[current];
                                 ...
                                 int					depth)
{
  ...
  if (portionSize == 1) { // Leaf case
    ...
    VALUE medianVal = (*initialEltList)[medianId].dim[dimIndex];
